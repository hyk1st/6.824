...
func (cfg *config) checkLogs(i int, m ApplyMsg) (string, bool) {
	...
	for j := 0; j < len(cfg.logs); j++ {
		if old, oldok := cfg.logs[j][m.CommandIndex]; oldok && old != v {
			log.Printf("%v: log %v; server %v, old is %v, v is %v\n", i, cfg.logs[i], cfg.logs[j], old, cfg.logs[i][m.CommandIndex])
	...
	for m := range applyCh {
		if m.CommandValid == false {
			...
			if m.CommandIndex > 1 && prevok == false {
				err_msg = fmt.Sprintf("server %v apply out of order %v prevok %v row 170", i, m.CommandIndex, prevok)
			}
			if err_msg != "" {
...
func (cfg *config) one(cmd interface{}, expectedServers int, retry bool) int {
	...
	if cfg.checkFinished() == false {
		cfg.t.Fatalf("one(%v) failed to reach agreement 603", cmd)
	}
	return -1
}
